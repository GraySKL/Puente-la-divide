// pscreens.jsx — Onboarding, Home, RightsCard, Phrasebook, ComingSoon, Discreet.

// ---------- ONBOARDING ----------
function Onboarding({ onDone, wide }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: C.bg, padding: wide ? '40px 48px' : '20px 26px 26px' }}>
      ...
      <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', maxWidth: 420, margin: '0 auto' }}>
        {!onConfirm ? (
          ...
          <React.Fragment>
            ...
            <div style={{ font: `800 26px/1.2 ${C.round}`, color: C.ink, marginTop: 22 }}>Soy Tía Marisol.<br />Vamos juntas.</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 22, width: '100%', maxWidth: 300 }}>
              {['🗣️  Hablo español', '🎯  Quiero aprender inglés'].map((t) => (
                <div key={t} style={{ background: '#fff', boxShadow: C.sh, borderRadius: 16, padding: '14px 18px', font: `800 15px ${C.round}`, color: C.ink, textAlign: 'left' }}>{t}</div>
              ))}
            </div>
          </React.Fragment>
        )}
      ...
      <div style={{ flex: '0 0 auto', maxWidth: 420, width: '100%', margin: '0 auto' }}>
        ...
        {!onConfirm && <div style={{ textAlign: 'center', marginTop: 12 }}><button onClick={onDone} style={{ border: 'none', background: 'none', cursor: 'pointer', font: `700 13px ${C.round}`, color: C.faint }}>Saltar</button></div>}
      </div>
    </div>
  );
}
...
// ---------- HOME ----------
function Home({ onPick, completed, wide }) {
  const cont = 'parada';
  const t = TOPICS[cont];
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: C.bg }}>
      <div style={{ padding: wide ? '8px 8px 24px' : '8px 22px 20px', maxWidth: wide ? 760 : 'none', margin: '0 auto' }}>
        ...
        {/* continue */}
        <div onClick={() => onPick(cont)} style={{ cursor: 'pointer', marginTop: 20, borderRadius: 22, background: topicSoft(t.hue), border: `1px solid ${topicEdge(t.hue)}`, padding: '18px 20px', boxShadow: C.sh }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ font: `800 11px ${C.round}`, letterSpacing: '1.1px', textTransform: 'uppercase', color: topicInk(t.hue) }}>{completed[cont] ? 'Repasar' : 'Seguir'} · {t.es}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16 }}>
            <div style={{ flex: 1 }}><ProgressBar value={completed[cont] ? 1 : 0.5} hue={t.hue} /></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: wide ? '1fr 1fr 1fr' : '1fr 1fr', gap: 12 }}>
          {APP_TOPICS.map((k) => {
            const tt = TOPICS[k]; const locked = LOCKED[k]; const done = completed[k];
            return (
              <div key={k} onClick={() => onPick(k)} style={{ cursor: 'pointer', position: 'relative', borderRadius: 20, background: '#fff', boxShadow: C.sh, padding: '15px 16px 17px', opacity: locked ? 0.72 : 1 }}>
...
// ---------- RIGHTS CARD (panic / quick access) ----------
function RightsCard({ onClose, topic = 'parada' }) {
  const [big, setBig] = React.useState(false);
  const hue = TOPICS[topic].hue;
  const enSize = big ? 27 : 21, esSize = big ? 18 : 15;
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: topicSoft(hue) }}>
      <div style={{ flex: '0 0 auto', padding: '12px 22px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={onClose} style={{ border: 'none', cursor: 'pointer', font: `700 13px ${C.round}`, color: topicInk(hue), background: '#fff', borderRadius: 999, padding: '7px 14px', boxShadow: C.shSoft }}>Cerrar</button>
      </div>
      ...
      <div style={{ flex: '0 0 auto', padding: '10px 22px 14px' }}>
        <div style={{ display: 'flex', gap: 10 }}>
          <Pill kind="primary" hue={hue} onClick={readAll} style={{ flex: 1 }}>🔊 Leer en voz alta</Pill>
          <Pill kind="soft" hue={hue} onClick={() => setBig((b) => !b)} style={{ flex: '0 0 auto' }}>{big ? 'Aa−' : 'Aa+'}</Pill>
        </div>
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: C.bg }}>
      <div style={{ padding: wide ? '8px 8px 24px' : '12px 22px 24px', maxWidth: wide ? 720 : 'none', margin: '0 auto' }}>
        ...
        <div style={{ font: `600 13px ${C.round}`, color: C.dim, marginTop: 4, marginBottom: 16 }}>Toca cualquier frase para escucharla.</div>
        {PHRASES.map((g) => (
              ...
              {g.items.map((p, i) => (
                <div key={i} onClick={() => speak(p.en)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, background: '#fff', borderRadius: 16, padding: '13px 16px', boxShadow: C.shSoft }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ font: `600 12px ${C.round}`, color: C.dim, marginTop: 1 }}>{p.es}</div>
                  </div>
                  <span style={{ width: 32, height: 32, borderRadius: '50%', flex: '0 0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center', background: topicSoft(TOPICS[g.topic].hue), fontSize: 14 }}>🔊</span>
                </div>
              ))}
            </div>
          </div>
...
// ---------- COMING SOON ----------
function ComingSoon({ topic, onClose }) {
  const t = TOPICS[topic]; const hue = t.hue;
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', background: topicSoft(hue), padding: 30 }}>
      ...
      <div style={{ marginTop: 24 }}><Pill kind="primary" hue={hue} onClick={onClose}>Volver al inicio</Pill></div>
    </div>
  );
}

// ---------- DISCREET COVER ----------
function DiscreetCover({ onUnlock }) {
  const [hint, setHint] = React.useState(true);
  React.useEffect(() => { const id = setTimeout(() => setHint(false), 2600); return () => clearTimeout(id); }, []);
  return (
    <div onDoubleClick={onUnlock} style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg,#cfe0ef,#eaf1f7)', color: '#33414f', cursor: 'pointer', position: 'relative' }}>
...
Object.assign(window, { Onboarding, Home, RightsCard, Phrasebook, ComingSoon, DiscreetCover });
